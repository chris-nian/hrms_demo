...
构建一个轻量级 HRMS 系统，用于企业快速部署和试用人力资源管理能力。系统需满足：

- 界面精美大气，商务专业风格（深蓝主色调）
- 单容器运行，无外部依赖
- 人员档案、组织职位、考勤台账、薪酬管理与状态机审批流
- 支持中英文切换
- 无登录，直接进入系统
...
| 页面 | 路由 | 功能 |
|------|------|------|
| 运营仪表盘 | `/` | 组织规模、出勤质量、流程待办、合同风险和薪酬覆盖 |
| 人员档案 | `/employees` | 员工列表、搜索筛选、详情、编辑、停用和恢复 |
| 组织架构 | `/departments` | 部门负责人、编制计划、职位维护和关联保护 |
| 考勤台账 | `/attendance` | 表格视图维护考勤记录、按月统计 |
| 薪酬管理 | `/salary` | 薪酬配置保存、税前/税后/社保/公积金/个税试算 |
| 流程中心 | `/approvals` | 审批列表、发起申请、审批操作、状态机记录 |
...
- 左侧：配置区（选员工、基本工资、奖金、扣款、社保/公积金比例）
...
- **身份切换**：页面顶部"当前身份"切换器（员工/主管/HR），用于当前轻量角色上下文
...
- 8 个部门（含负责人、职责说明和编制计划）
- 多个职位（含职级、描述和编制计划）
- 40-80 个员工（含员工编号、组织关系、主管、合同与状态）
- 当月考勤记录
- 多条不同状态的审批流
- 每个员工的薪酬配置
